                ...
                <div class="card-body">
                    <h5 class="card-title">{{$space->title}}
                        ...
                        <form action="{{route('space.destroy', $space->id)}} " method="post">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-danger float-right ml-1" onclick="return confirm('Yakin dihapus ?');"> Delete</button>
                    ...
                    <p class="card-text">{{$space->desciption}}</p>
